        ...
        <div id="source-doc-wrapper">
            ...
            <div class="jig-tabs">
                ...
                <div id = "tabs-2" class = "content">
                    ...
                    <div class = "content-footnote"><p><em>*as of <xsl:value-of select = "$release"/> UMLS.<br/>Source vocabularies in Category 4 are free for use in the United States. Category 3 rules apply for all other uses</em></p></div>
                </div>
                <!--
    ...
    <xsl:template match = "letter">
        <xsl:if test = "count(sources/source) gt 0">
            ...
            <div class = "sourcecontainer">
                <table border = "0" summary = "Table of sources beginning with the letter {@name}">
    ...
    <xsl:template match = "restriction">
        <xsl:choose>
            <xsl:when test = "child::letters">
                <h5><a class="jig-ncbitoggler" href="#"><xsl:value-of select = "@name"/>&#160;&#160;<span class = "count"><xsl:value-of select = "count(letters/letter/sources/source)"/>&#160;<xsl:value-of select = "sourcereleasedocs:countSources(count(letters/letter/sources/source))"/></span></a></h5>
                <div class = "sourcecontainer">
            ...
            <xsl:otherwise>
                <xsl:if test = "child::sources/source">  
                    <h5><a class="jig-ncbitoggler" href="#"><xsl:value-of select = "@name"/>&#160;&#160;<span class = "count"><xsl:value-of select = "count(sources/source)"/>&#160;<xsl:value-of select = "sourcereleasedocs:countSources(count(sources/source))"/></span></a></h5>
                    <div class = "sourcecontainer">
                        <table border = "0" summary = "Table of restriction level {@name} sources">
                            <tr><th scope = "col">Source</th><th>Last Updated</th></tr>
                            <xsl:apply-templates select = "sources"/>
                        </table>
                    </div>
...
<!-- end restriction area of document -->
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test = "child::sources/source">  
                    <h5><a class="jig-ncbitoggler" href="#"><xsl:value-of select = "@name"/>&#160;&#160;<span class = "count"><xsl:value-of select = "count(sources/source)"/>&#160;<xsl:value-of select = "sourcereleasedocs:countSources(count(sources/source))"/></span></a></h5>
                    <div class = "sourcecontainer">
                        <table border = "0" summary = "Table of {@name} language sources">
                            <tr><th scope = "col">Source</th><th>Last Updated</th></tr>
                            <xsl:apply-templates select = "sources"/>
                        </table>
                    </div>
    ...
    <xsl:template match = "sources">
        ...
        <xsl:for-each select = "source">
            ...
            <tr>
                <td valign = "top"><a href = "http://www.nlm.nih.gov/research/umls/sourcereleasedocs/{$release}/{translate(current(),'.','')}" target = "_blank"><xsl:value-of select = "."/> (<xsl:value-of select = "@ssn"/>)</a></td>
