...
Once new content is finalized, sync it with the production folder (/jbake/content/) by running the following command.

bash scripts/syncLatestVersion.sh

Check to make sure the content updated properly, especially if there is a new folder. 
...
UPDATING METADATA.HTML FILES, TITLES, DATES, MENUS
...
Once this process is done, the site should be ready for release. Run the site locally and QA it extensively. Look especially at new content to make sure that it updated properly. Make sure that there are no problems with menus. Check dates. Scripts will not necessarily give useful errors. 
